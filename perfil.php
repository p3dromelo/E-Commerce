...
// Preparar a consulta
$query = $conn->prepare("SELECT * FROM historico_compras WHERE user_email = ? ORDER BY data_compra DESC");
if (!$query) {
...
<div class="history-container">
    ...
    <?php if (empty($historico)) : ?>
        <p class="no-history">Nenhuma compra registrada até o momento.</p>
    <?php else : ?>
            ...
            <tbody>
                <?php foreach ($historico as $compra): ?>
                    <tr>
                        ...
                        <td><?= ucfirst($compra['tipo_produto']) ?></td>
                        <td><?= $compra['quantidade'] ?></td>
                        <td>R$ <?= number_format($compra['preco_unitario'], 2, ',', '.') ?></td>
                        <td><?= ucfirst($compra['metodo_pagamento']) ?></td>
